...
class AttentionBlockAnimationMasked(Scene):
    def construct(self):

        title = Text("Masked Self Attention Block", font_size=36)
        ...
        horizontal_text.next_to(title, DOWN, buff=0.5)  # Changed this line to position horizontal_text below the title
        ...
        num_words = len(words)
        attention_block = EmbeddingTable(rows=num_words, columns=num_words, color=BLUE, title="", scale=1, indexed=False, tril = True)
        attention_block.next_to(vertical_text, RIGHT, buff=1)
        attention_block.next_to(horizontal_text, DOWN, buff=1)

        # Animation
        self.play(Write(title), Write(horizontal_text), Write(vertical_text))
        ...
        for i in range(num_words):
            for j in range(num_words):
                cell_text = attention_block.table_group[i][1][j].text
                if cell_text != "-∞":
